...
public class EmployeeCriteriaRepository {
    ...
    public Page<Employee> findAllWithFilters(EmployeePage employeePage, EmployeeSearchCriteria searchCriteria){
        // create criteriaQuery, declares the type of row in the result
        // select
    ...
    private Predicate getPredicate(EmployeeSearchCriteria employeeSearchCriteria, Root<Employee> employeeRoot) {
        ...
        if (Objects.nonNull(employeeSearchCriteria.getFirstName())) {
            predicates.add(criteriaBuilder.like(
                    employeeRoot.get("firstName"), "%" + employeeSearchCriteria.getFirstName() + "%")
            );
        }
        if (Objects.nonNull(employeeSearchCriteria.getLastName())) {
            predicates.add(criteriaBuilder.like(
                    employeeRoot.get("lastName"), "%" + employeeSearchCriteria.getLastName() + "%")
            );
        }
